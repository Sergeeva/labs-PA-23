...
bool check_res(const Matrix& a, vector<cl_int> b) 
{
...
unsigned long create_kernel(cl_kernel kernel, cl_command_queue queue, cl_mem& A, cl_mem& B, cl_mem& buff,
...
int main() 
{
	...
	auto gpu_time = create_kernel(kernel, queue, A, B, buff, result.data(), MATRIX_SIZE);
