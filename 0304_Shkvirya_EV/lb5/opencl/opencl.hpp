...
public:
    ...
    void createImage(int width, int height, int squareSize) {
        ...
        size_t global[2] = {(size_t)width, (size_t)height}; // задаём глобальный размер изображения  (вся задача)
        size_t local[2] = {(size_t)squareSize, (size_t)squareSize}; // задаём локальный размер обрабатуемого блока изображения  (рабочая группа)
