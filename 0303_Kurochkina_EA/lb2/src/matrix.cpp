...
int sum(const vector<vector<int>>& left, const vector<vector<int>>& right, int i, int j) {
...
std::ostream& operator<< (std::ostream& file, const Matrix& m) {
    file << m.rows << ' ' << m.cols << '\n';
