...
void Flows::multiplier() {
    for(int k = 0; k < this->iterationsConsumer; k++) {
        std::pair<Matrix, Matrix> matrixs;
        while(true){
            std::optional<std::pair<Matrix, Matrix>> result = queueMatrix.pop();
            if(result.has_value()){
                matrixs = result.value();
                break;
            }
        }
...
void Flows::writer() {
    for(int i = 0; i < this->iterationsProducer; i++){
        Matrix m;
        while(true){
            std::optional<Matrix> result = queueResult.pop();
            if(result.has_value()){
                m = result.value();
                break;
            }
