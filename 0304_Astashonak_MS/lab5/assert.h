...
// Code below used for debugging, for easier exception control
...
std::string clGetErrorString(cl_int const err) {
    switch (err) {
...
cl_int cl_assert(cl_int const code, const std::string& file, int const line, bool const abort) {
    if (code != CL_SUCCESS) {
        std::string err_str = clGetErrorString(code);

        std::cerr << "\"" << file << "\", line " << line << ": cl_assert (" << code << ") = \"" << err_str << "\"" << std::endl;
